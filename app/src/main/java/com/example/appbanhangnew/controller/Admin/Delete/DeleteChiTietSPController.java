...
public class DeleteChiTietSPController {
    Context context;

    public DeleteChiTietSPController(Context context) {
    ...
    public void DeleteChiTietSP(ChitietSP chitietSP){
        new DeleteChiTietSP(chitietSP).execute(Server.duongdandeleteChiTietSP);
    }

    class DeleteChiTietSP extends AsyncTask<String,Void,String> {
        OkHttpClient okHttpClient = new OkHttpClient().newBuilder().build();
        ChitietSP chitietSP;

        public DeleteChiTietSP(ChitietSP chitietSP) {
            this.chitietSP = chitietSP;
        }
        ...
        protected String doInBackground(String... strings) {
            ...
            Request request= new Request.Builder()
                    //Truyền url;
                    .url(Server.duongdandeleteChiTietSP).post(requestBody).build();
            try {
                //Trả giá trị dề; Thực hiện request;
                Response response=okHttpClient.newCall(request).execute();
                return response.body().string();
