...
import com.example.appbanhangnew.action.Admin.FragmentAdmin.MeAdminFragment;
...
public class FragmentAdminAdapter extends FragmentPagerAdapter {
    Context context;


    public FragmentAdminAdapter(@NonNull FragmentManager fm, int behavior, Context context) {
    ...
    public Fragment getItem(int position) {
        switch (position){
            case 0:
                return new HomeAdminFragment(context);
            case 1:
                return new LienHeFragment();
            case 2:
                return new MeAdminFragment(context);
